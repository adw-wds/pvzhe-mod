namespace PvzheMod
{
    ...
    {
        public static bool Enabled = false;    // 是否启用彩色（默认关，降低渲染开销；需要彩虹再开）
        ...
        public static bool NoCostRise = false;            // 卡价不涨价（金卡/重复购买 costRise=0）
        public static bool ZeroCost = false;              // 零消费（所有卡免费，GetCost=0）
        public static string BulletType = "";            // 自定义子弹类型（PROJECTILE_CONFIG key，空=不生效）
        public static bool CanChooseAll = false;          // 所有关卡可选卡（强制 packetBankMethod=CHOOSE）
        ...
        public static bool BossNoBow = false;             // 僵王不低头（HeadExited 拦截）
        public static bool NoThrowImp = false;            // 巨人禁止投掷小鬼（ImpThrower.SpawnImp 拦截）
        public static bool PerfMode = false;              // 性能优化模式（降低渲染/遍历开销，进游戏卡时开）
        public static int TextColorMode = 0;              // 文字颜色：0=彩虹律动 1=纯白 2=纯黑
        public static bool CannonNoCooldown = false;       // 炮类无冷却（玉米加农炮等 CannonComponent）
        public static bool IgnoreHouse = false;            // 无视僵尸进家（进家不失败）
        public static bool IgnoreWarningLine = false;      // 僵尸碰到警戒线不失败
        public static bool PlantSquash = false;            // 趣味：植物有节奏地被压扁/拉扁
        ...
        public static bool PlantFullRange = false;           // 全植物全图射程（任何位置都能攻击全图）
        ...
        public static bool BungiIgnoreUmbrella = false;        // 飞贼无视保护伞：保护伞挡不住飞贼偷取
        public static bool JackboxFastBomb = false;            // 小丑秒炸：小丑瞬移到目标面前立即爆炸（敌方炸植物/被魅惑炸僵尸）
        public static bool ZombiesFollowMouse = false;         // 全场僵尸吸附到鼠标（僵尸被拉向鼠标位置）
        public static bool BungiSpawnJackbox = false;          // 飞贼偷取后原地生成一个快要爆炸的小丑
        ...
        public static bool TrickRain = false;          // 篡改种子雨：天上掉的卡 = 目标池（仅掉卡型雨，掉阳光型不适用）
        ...
        public static string TrickPool = "";          // 目标卡池（逗号/空格/换行分隔的植物或僵尸 ID；留空=不篡改）
        public static bool SeedBankEveryFrame = true;  // 卡槽卡牌随机：true=1帧1刷（极速闪换）；false=约每秒6次
        public static bool EngineLowRes = false;       // 低配渲染：把窗口渲染分辨率降到 1280 宽（stretch 等比缩放，帧率显著提升）
        /// <summary>是否手动保存过（存档文件带 #manual 标记）。启动时未手动保存过 → 全部功能默认关闭。</summary>
        public static bool ManualSaved;
        private static readonly string SavePath = @"mod\modsettings.txt";
        public static readonly string SaveDir = @"mod\";   // 其他存档文件目录（plantmods.json 等）
        ...
        public static void Save()
        {
            try
            {
                ...
                sb.AppendLine("#manual");   // 手动保存标记：下次启动恢复该配置（否则默认全部关闭）
                ...
                sb.AppendLine("EngineLowRes=" + EngineLowRes);
        ...
        public static void Load()
        {
            ...
            try
            {
                ...
                fa.Close();
                if (all.IndexOf("#manual", System.StringComparison.Ordinal) >= 0) ManualSaved = true;
                foreach (var line in all.Split(new[] { "\r\n", "\n" }, System.StringSplitOptions.None))
                {
                    ...
                    switch (k)
                    {
                        ...
                        case "EngineLowRes": if (bool.TryParse(v, out b)) EngineLowRes = b; break;
        ...
        /// <summary>恢复全部字段为默认值（"一键全部关闭"用）。</summary>
        public static void ResetAll()
        {
            Enabled = false; Speed = 0.4f; Style = 0;
            FunEnabled = true; BGEnabled = true; GameSpeed = 1.0f;
            CustomMessages = "";
            InfiniteSun = false; InfiniteCoin = false;
            ESPEnabled = false; ESPZombie = true; ESPPlant = false;
            ZombieScale = 1.0f; PlantScale = 1.0f; VaseESP = true;
            DanmakuSpeed = 1.0f;
            NoCooldown = false; AlmanacAll = false;
            PlantOverlap = true; IgnoreTerrain = true; IgnorePurple = false;
            PlantAttackSpeed = 1.0f; ZombieAttackSpeed = 1.0f;
            PlantHP = 1.0f; ZombieHP = 1.0f;
            PlantInvincible = false; ZombieInvincible = false;
            CharmPlant = false; CharmZombie = false;
            BulletTrack = false; BulletFollowMouse = false; BulletRandom = false;
            PlantAutoFire = false; FogESP = false; PlantColumn = false;
            NoSleep = false; ClearCrater = false; ConveyorFast = false; RainFast = false;
            VaseRandom = false; SeedBankRandom = false;
            ShovelCherry = false; BloverClearAll = false; CannonMultiShot = 1;
            ZombieColor = false; ZombieDance = false;
            PlantNoAttack = false; ZombieNoAttack = false; ZombieNoMove = false;
            GloveMode = false; PlantTriple = false; NoCostRise = false; ZeroCost = false;
            BulletType = ""; CanChooseAll = false; NoZombieSpawn = false;
            BossNoBow = false; NoThrowImp = false;
            PerfMode = false; TextColorMode = 0;
            CannonNoCooldown = false; IgnoreHouse = false;
            IgnoreWarningLine = false; PlantSquash = false; JellyMode = false;
            ForceRain = false; ForceFog = false; IgnoreRedLine = false; SeedBankRandomZombie = false;
            ShovelCherry = false;
            BungiFastGrab = false; BungiIgnoreUmbrella = false;
            JackboxFastBomb = false; ZombiesFollowMouse = false; BungiSpawnJackbox = false;
            ChomperFastSwallow = false;
            GlobalOverridesEnabled = false;
            TrickEnabled = false; TrickBox = false; TrickRain = false; TrickConveyor = false;
            TrickFixedOnly = false; TrickPool = ""; SeedBankEveryFrame = true; EngineLowRes = false;
        }

        /// <summary>彻底全关：所有功能开关（含默认开的美化项）复位 + 数值复位。启动时调用，默认全部关闭。</summary>
        public static void ForceAllOff()
        {
            foreach (var f in typeof(ModSettings).GetFields(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static))
                if (!f.IsLiteral && f.FieldType == typeof(bool))
                    f.SetValue(null, false);
            GameSpeed = 1f; PlantAttackSpeed = 1f; ZombieAttackSpeed = 1f;
            PlantHP = 1f; ZombieHP = 1f; ZombieScale = 1f; PlantScale = 1f;
            CannonMultiShot = 1; SpawnMultiplier = 1; BulletType = ""; CustomMessages = "";
        }
        ...
        public static bool AnyModActive()
        {
            return InfiniteSun || InfiniteCoin || NoCooldown || AlmanacAll || IgnorePurple ||
                   ...
                   NoCostRise || ZeroCost || BulletType.Length > 0 || CanChooseAll ||
