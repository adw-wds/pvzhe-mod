    ...
    public static class ESP
    {
        ...
        public static void OnFrame(Node root)
        {
            try
            {
                ...
                {
                    // 关闭透视/缩放时：先清除已画的框（否则取消勾选后框还在）
                    if (_drawn.Count > 0)
                    {
                        foreach (var d in _drawn) if (GodotObject.IsInstanceValid(d)) d.QueueFree();
                        _drawn.Clear();
                    }
                    // 诊断模式：不开功能也每 100 帧扫描一次并记录角色识别结果
                    if (++_frame % 3 != 0) return;
                    if (++_logTimer >= 100)
                    {
                        _logTimer = 0;
                        // 诊断扫描（不再写日志，避免高频 IO 卡顿）
                        _zombieCount = 0; _plantCount = 0; _node2dCount = 0; _charCount = 0;
                        Walk(tree.Root, false, false);
                    }
                    return;
                }
